...
let degree fmt = chout ConsoleColor.Cyan "degree" fmt
...
let interpreter_loop () =
    while true do
        ...
        try
            ...
            | Cmd ("nome" | "alunno" | "studente") ->   // Identificazione dello studente
                out "%s" "Melania Gottardo"
            ...
            // EXPR
            // Il caso Expr e1 restituisce:
            //    • la forma ridotta, ovvero con lo svolgimento delle derivate
            //    • la forma normalizzata, ovvero la forma semplificata e ordinata (in ordine crescente dell'esponente della variabile x)
            //    • il grado del polinomio normalizzato

            | Expr e1 -> redux  "%O" (Impl.reduce e1)
                         norm   "%O" (Impl.normalize (Impl.reduce e1))
                         degree "%O" (Impl.normalized_polynomial_degree (Impl.normalize (Impl.reduce e1)))                       

            // EQU
            // Il caso Equ (e1, e2) restituisce:
            //     • la forma ridotta delle due espressioni
            //     • la forma normalizzata in cui vengono unite le due espressioni a sinistra dell'uguale
            //     • il grado del polinomio normalizzato a sinistra dell'uguale
            //     • nel caso si tratti di un'identità, viene restituito il corrispondente booleano
            //       altrimenti vengono restituite le soluzioni dell'equazione (massimo grado = 2)

            | Equ (e1, e2) -> redux  "%O = %O" (Impl.reduce e1) (Impl.reduce e2)
                              norm   "%O = 0"  (Impl.normalize (Impl.equazione (e1, e2)))
                              degree "%O"      (Impl.normalized_polynomial_degree (Impl.normalize (Impl.equazione (e1, e2))))
                              match Impl.normalized_polynomial_degree (Impl.normalize (Impl.equazione (e1, e2))) with
                              | 0 -> ident "%O"   (Impl.solve0 (Impl.normalize (Impl.equazione (e1, e2))))
                              | 1 -> sol "x = %O" (Impl.solve1 (Impl.normalize (Impl.equazione (e1, e2))))
                              | 2 -> match (Impl.solve2 (Impl.normalize (Impl.equazione (e1, e2)))) with
                                     | None                 -> sol "L'equazione non ammette soluzioni reali"
                                     | Some (x1, None)      -> sol "x = %f" x1
                                     | Some (x1, Some (x2)) -> sol "x1 = %f  vel  x2 = %f" x1 x2
                              | _ -> error "Il grado dell'equazione non è supportato"                          
